...
reducer takes new input and adds it to the older data to make a kind of on going history - it does not directly change the data. it brings new data together with the old to create the present data. actions are the way that data is added to the reducer. store is where all the data produced by the reducer is stored for use by the app.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

application state is usually used by multiple components and is held at a high level above the components that are going to use it. component data is used by a single component and is held by that component.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

thunk allows for side effect logic that is needed for synchronous logic - like AJAX requests. thunk allows an action-creator run logic and still interact with the store.
